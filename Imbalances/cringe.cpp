/** 
 * This code is not supposed to exist.
 */
...
class Table {
    int n, *T; 
public: 
	Table(int _n) : n(_n) { T = new int[(n + 2) * (n + 2) + 69]; }
	int& operator [] (const office &P) const {
		return T[P.first * (n + 2) + P.second]; 
	}
	~Table() {
	    delete [] T;
	}
...
signed main() {
	...
	for (int i = 1; i <= N; i++) {
		...
		amount[{0, i}] = amount[{i, 0}] = amount[{i, N + 1}] = amount[{N + 1, i}] = INT_MAX; 
	}
	int ans = 0; 
	for (auto middle : pairs(1, N, 1, N)) {
		for (auto [dir_1, dir_2] : pairs(0, 3, 0, 3)) {
			if (dir_1 >= dir_2) continue;  // direction pairs are unordered
			ans += (amount[middle] 
